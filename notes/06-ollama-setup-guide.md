...
```bash
# ---- 启动 ----
ollama serve                  # 前台启动（终端关掉服务就停，适合调试）
brew services start ollama    # 后台启动 + 设为开机自启动（推荐）

# ---- 停止 ----
# 如果是 ollama serve 前台运行的 → 直接 Ctrl+C
# 如果是 brew services 启动的：
brew services stop ollama     # 停止服务（不会删模型）

# 如果上面的都不管用（进程卡死/端口冲突）：
killall ollama                # 强杀所有 ollama 相关进程
# 或按端口杀：
lsof -i :11434 | grep LISTEN | awk '{print $2}' | xargs kill

# ---- 状态 ----
brew services list | grep ollama   # 查看 ollama 服务是否在运行
curl http://localhost:11434        # 返回 "Ollama is running" 说明服务正常
...
```bash
# ---- 拉取 / 删除 / 列表 ----
ollama pull <model>           # 拉取模型（如 ollama pull qwen3:8b）
ollama list                   # 列出本地已下载的所有模型
ollama rm <model>             # 删除模型（释放磁盘空间）
ollama cp <src> <dst>         # 复制模型（用于备份自定义 Modelfile）
ollama show <model>           # 查看模型详细信息（参数量、量化方式、架构、Modelfile）

# ---- 查看模型文件占用 ----
du -sh ~/.ollama/models/      # 所有模型占用的磁盘空间
ls ~/.ollama/models/blobs/    # 模型的实际 GGUF 二进制文件（sha256 命名）

# ---- 升级模型 ----
ollama pull <model>           # 重复执行 pull 会检查远程是否有更新版本
```
...
```bash
# ---- 查看 ----
ollama ps                     # 查看当前加载到内存的模型（含 GPU 使用率、剩余 keep-alive 时间）
# NAME            ID              SIZE      PROCESSOR    UNTIL
# qwen3.5:9b      xxxxxxxx        5.1 GB    100% GPU    4 minutes from now

# ---- 手动卸载模型 ----
ollama stop <model>           # 立即卸载指定模型，释放显存（不等 keep-alive 超时）
ollama stop qwen3.5:9b        # 例子

# ---- 修改 keep-alive 时长 ----
OLLAMA_KEEP_ALIVE=1h ollama run qwen3:8b "测试"    # 单次：保留 1 小时
OLLAMA_KEEP_ALIVE=-1 ollama run qwen3:8b "测试"     # 一直驻留，永不自动卸载
# 或者写进环境变量（永久生效）：
export OLLAMA_KEEP_ALIVE=30m    # 默认 5m；-1 表示永不卸载；0 表示回复完立即卸载
```
...
```bash
# ---- 交互式对话 ----
ollama run <model>            # 进入 REPL 交互模式
# 进入后可以输入对话，输入 /bye 退出，输入 /? 查看帮助

# ---- 单次问答 ----
ollama run <model> "用一句话解释什么是 TCP 三次握手"
echo "你好" | ollama run <model>       # 从管道传入 prompt

# ---- 指定参数 ----
# CLI 方式：ollama run 0.30.x 起不再支持 --temperature / --num-predict
# 控制参数推荐两种方式：
# 方式 1：API 调用（推荐，也是你 agent 代码中的实际用法）
#   在 chat/completions 请求中传 temperature / max_tokens
# 方式 2：通过 Modelfile 创建固定参数的自定义模型
ollama create my-model -f MyModelfile   # 文件中写 PARAMETER temperature 0.2
```

### 7.5 自定义模型 (Modelfile)

```bash
# 创建一个自定义模型（修改系统提示词 / 温度等默认参数）
# 1. 写一个 Modelfile：
cat > WifeModelfile << 'EOF'
FROM qwen3.5:9b
SYSTEM "你是一只用户的猫娘老婆。用撒娇、粘人的语气回答。"
...
```bash
# 常用环境变量（通过 launchctl setenv 或 export 设置）
OLLAMA_HOST=0.0.0.0:11434        # 允许局域网其他设备访问（默认 127.0.0.1）
OLLAMA_NUM_PARALLEL=4            # 每个 Runner 同时处理的最大请求数
OLLAMA_MAX_LOADED_MODELS=1       # 同时驻留内存的模型数量上限（默认 3×GPU 数）
OLLAMA_KV_CACHE_TYPE=q8_0        # KV Cache 量化（f16 / q8_0 / q4_0）
OLLAMA_FLASH_ATTENTION=1         # 开启 Flash Attention
OLLAMA_KEEP_ALIVE=30m            # 模型驻留时间（默认 5m，-1 表示永久）
OLLAMA_MMAP=1                    # 启用内存映射加载（默认开启，省内存）
...
```bash
# ---- 端口冲突 ----
lsof -i :11434                 # 看看谁在占用 11434 端口
lsof -i :11434 | grep LISTEN | awk '{print $2}' | xargs kill   # 杀掉占用端口的进程

# ---- 查看日志 ----
# brew services 启动的：
tail -f /opt/homebrew/var/log/ollama.log    # macOS Homebrew 日志路径
# 或者
journalctl -u ollama -f                      # Linux systemd 日志

# ---- 模型损坏 / 下载中断 ----
ollama rm <model>              # 删掉坏的
ollama pull <model>            # 重新拉

# ---- 确认是 ollama 的问题还是模型的问题 ----
...
| 来源 | 类型 | 链接 |
|---|---|---|
| Ollama 官方源码（Go） | 源码仓库 | https://github.com/ollama/ollama |
| Ollama 官方文档首页 | 官方文档 | https://ollama.com |
| Ollama 官方模型库 | 模型索引 | https://ollama.com/search |
| Ollama GitHub README（含 CLI 命令参考） | CLI 文档 | https://github.com/ollama/ollama#readme |
| Ollama 官方 FAQ | FAQ | https://github.com/ollama/ollama/blob/main/docs/faq.md |
| Ollama 官方 Modelfile 文档 | 自定义模型 | https://github.com/ollama/ollama/blob/main/docs/modelfile.md |
| Ollama 环境变量 / 配置文档 | 配置参考 | https://github.com/ollama/ollama/blob/main/docs/faq.md#how-can-i-configure-ollama |
